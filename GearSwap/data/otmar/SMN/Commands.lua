...
function Commands.PetMap(commandargs)
    ...
    if commandargs[3] == "me" then
        cmd = '/pet "'..Abilities[commandargs[2]].name..'" <me>'
        Utils.Input(cmd)
        Utils.Mirror(cmd)
    elseif commandargs[3] == "t" then
        target = windower.ffxi.get_mob_by_target('<t>').id
        cmd = '/pet "'..Abilities[commandargs[2]].name..'" '..target
        Utils.Input(cmd)
        Utils.Mirror(cmd)
    elseif commandargs[4] then
        target = windower.ffxi.get_mob_by_target('<lastst>').id
        ...
        Utils.Mirror(cmd)
    elseif commandargs[3] then
        cmd = '/pet "'..Abilities[commandargs[2]].name..'" '..commandargs[3]..''
        Utils.Input(cmd)
        Utils.Mirror(cmd)
    else
        cmd = '/pet "'..Abilities[commandargs[2]].name..'" <stnpc>'
        Utils.Input(cmd)
        Utils.Mirror(cmd)
    end
end

function Commands.Summon(commandargs)
    cmd = '/ma '..CurrentAvatar..' <me>'
    Utils.Input(cmd)
    Utils.Mirror(cmd)
end
...
function Commands.AvatarHelp(commandargs)
    vals = {"Phys1","Phys2","MB1","MB2","Alt1","AoE","Buff1","Buff2","Buff3","Buff4","Debuff1","Debuff2","Astral","Favor"}
    Utils.Echo('\'')
