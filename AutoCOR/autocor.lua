    ...
    if nexttime + del <= curtime then
        ...
        for x = 1,2 do
            ...
            if not buffs[roll.buff] then
                ...
                return
            elseif buffs[308] and buffs[308] == roll.id and has11() and buffs[roll.buff] ~= 11 then
                if abil_recasts[197] and abil_recasts[197] == 0 and not buffs[357] and buffs[roll.buff] == 10 then
                    use_JA('/ja "Snake Eye" <me>')
                elseif abil_recasts[194] and abil_recasts[194] == 0 then
...
function has11()
    for x = 1,2 do
        local roll = rolls:with('en',settings.roll[x])
        if buffs[roll.buff] and buffs[roll.buff] == 11 and not buffs[309] then
            return true
        end
    end
    return false
